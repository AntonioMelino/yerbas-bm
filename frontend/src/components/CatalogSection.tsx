...
export default function CatalogSection({
  ...
  const chipClass = (active: boolean) =>
    `px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-full text-xs sm:text-sm border transition-all duration-300 ${
      active
  ...
  return (
    <section id="catalogo" className="max-w-7xl mx-auto px-4 sm:px-5 md:px-8 py-16 sm:py-24">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-8 sm:mb-10">
        ...
        <div className="relative">
          <input
            ...
            className="bg-forest/60 border border-olive/60 rounded-full pl-10 sm:pl-11 pr-4 sm:pr-5 py-2.5 sm:py-3 text-sm w-full sm:w-64 text-cream placeholder:text-cream/40 focus:outline-none focus:border-gold transition"
          />
          <svg
            className="w-4 h-4 absolute left-3.5 sm:left-4 top-1/2 -translate-y-1/2 text-cream/40"
            fill="none"
            stroke="currentColor"
            strokeWidth={2}
            viewBox="0 0 24 24"
